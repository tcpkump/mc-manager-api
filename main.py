...
def get_mcbot_data(server: str) -> dict:
    ...
    if os.path.isfile(mcbot_data_file):
        ...
        logging.warning("Did not find mcbot config file")
        return {}
...
def list():
    try:
        ...
        directories = [
            item for item in items if os.path.isdir(os.path.join(SERVERS_DIR, item))
        ]
    ...
    try:
        ...
        subprocess.Popen(
            [
                "docker-compose",
                "-f",
                f"{SERVERS_DIR}/{server}/docker-compose.yml",
                "up",
                "-d",
            ]
        )
...
Please allow time for the server to start.
If you disconnect, it will automatically shut down after 36 hours.
        """
